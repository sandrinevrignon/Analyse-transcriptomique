...
def fichier_assemblage_trinity_PE():
    ...
    #Création d'un fichier d'analyse bash copiant le fichier brut et contenant les modification du chemin et des données 
    ...
    for k in lect:
        #Ecriture des lignes ne contenant aucune des variables ci-dessous
        if k!="modifvariableleft\n" and k!="modifvariableright\n" and k!="modifvariable\n" :
            assemblage.write(k)
        else:
            #Remplacement de la variable par les données échantillons
            if k=="modifvariableleft\n":
                assemblage.write(f" --single {modifvariableleft[0]}")
